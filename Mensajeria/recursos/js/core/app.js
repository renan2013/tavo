...
(function(window, document, $) {
    ...
    $(window).on('load',function(){
		...
		let profile=traerLocalStorage()
		console.log("profile",profile)
		  
		
		try
		{
			if ( profile ==0) //volver a raiz 
			{ 
				//window.history.go(-1);
				window.location.assign("http://tavocr.com")
			}
		 } catch (e) 
		 {
			  window.history.go(-1);
		 ...
		 console.log("profile",profile[0].Roles)
		if (profile[0].Roles.indexOf("TAVO_MENSAJERIA")==-1)
		  { 
				//window.history.go(-1);
				window.location.assign("http://tavocr.com")
		   }
		
		//recupera de localstorage el profile
