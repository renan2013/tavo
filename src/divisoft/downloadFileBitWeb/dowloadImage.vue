  ...
  <div>
    ...
    <div class="demo-alignment" v-show="cargando" type="gradient" color="success" >
     Cargando imagen....
      ...
      <div> 
            <img class="responsive rounded"  :src="textBase64" alt="user-upload" >
...
import * as divilib from "@/divisoftlibs/utilDivisoftJSBitWeb.js";
export default {
  created() {
    ...
    eventBus.$on("cargaDoc", PROPSDataCMS => {
      ...
      if (
        ...
        this.textBase64 = "";
        this.$store.dispatch("BitWeb/CMSacDsoaLimpia", divilib.DataCMS[0]);
      }
    });
  },
  reDestroy: function() {
  ...
  methods: {
    ...
    downloadFile() {
      this.DataCMS[0].CodCia = this.getCodCia;
      ...
      if (this.DataCMS[0].LlaveExterna > "0")
          { 
            console.log("enviando",JSON.stringify(this.CMSpeticion[0]));
            this.$store.dispatch("BitWeb/CMSacDsoa", this.CMSpeticion[0]);
