    ...
    <VuePerfectScrollbar class="scroll-area--data-list-add-new pt-4 pb-6"> 
        <vx-input-group class="mb-base">
            <div class="vx-row"> 
                                 ...
                                 <p>Código</p>
                                <vs-input size="large" name="COD_CIA" v-model="COD_CIA" class="mt-5 w-full" />
                                <span class="text-danger">{{errors.first("step-1.COD_CIA")}}</span>
                              </div><div class="vx-col w-1/2">
      <p>Compañia</p>
        <vs-input size="large" v-validate="'required'"    name="NOM_CIA" v-model="NOM_CIA" class="mt-5 w-full" />
         
        <span class="text-danger">{{errors.first("step-1.NOM_CIA")}}</span>
      </div>
            </div> 
...
import * as datos2500 from "./js/divi2500.js";
...
const dict = datos2500.mensajesValidacion;
...
export default {
  ...
  beforeMount() {
    ...
    this.defaultdatos = datos2500.datosDefault;
  ...
  watch: { 
   ...
   lastDbmsgCode:function(){
    ...
    currentRecord: function() {
      if (this.currentRecord.COD_CIA > "0")
  ...
  data() {
    return {
      ...
      // CMS CMS CMS CMS          DatOS cms Para manejo de EXPEDIENTE DIGITAL  MULTIMEDIA Debe venir en una funcion de Datos con la consulta
        // CMSDATA2500: datos2500.CMSDATA2500,  PROPSDataCMS: divilib.DataCMS,  tituloAvatar: "Para Cambiar Avatar",  verDocumento1: false,
 ...
 validateStep3() {
   ...
   //cargaRegistroActual funcion completa   
    ...
    buscarRegistro: function(dinamicKey) {
      ...
      var origenUrl = "O";
       // agrego las filas
         var filas = datos2500.getMisFilas();
