...
<template>
  <div>
    <div v-if="isNinja" id="GRID2507">
      <table class="table">
        <tbody>
          <tr>
            <td>
              <button class="btn btn-primary btn-sm" @click="RegistroNuevo">Agregar Registo</button>
              <button class="btn btn-primary btn-sm" @click="Pruebarouter">Prueba Router</button>
            </td>
            <td>
              <div
      ...
      <filtrosDivisoft
        v-show="activaFiltro"
        :header="locallastheader"
        :isSidebarActive="activaFiltro"
        :filas="locallaststrFilas"
        :filtros="filtros"
        @closeSidebar="activaFiltro = false"
...
export default {
  ...
  beforeMount() {
    ...
    this.columnDefs = datos2507.columnDefs;
    var defaultdatos = datos2507.datosDefault;
  ...
  methods: {
    ...
    onDoubleClicked(event) {
      ...
      this.dinamicKey.push({
        nombre: "COD_MONEDA",
        valor: event.node.data.COD_MONEDA
      });
    ...
    addFilas: function() {
      ...
      filas.push({ NOMBRE: "DES_MONEDA", VALOR1: "", VALOR2: "", CDATA: "0" });
      filas.push({ NOMBRE: "DES_PAIS", VALOR1: "", VALOR2: "", CDATA: "0" });
    ...
    RegistroNuevo() {
      ...
      this.$store.commit("MUTSETDATOSDEFAULT", defaultdatos);
      //LLAVES
      this.dinamicKey.push({ nombre: "COD_MONEDA", valor: "0" });
      //llama método editar registro
      eventBus.$emit("cargaRegistro", this.dinamicKey);
