...
<template>
  <vx-card :title="getAction" code-toggler>
    ...
    <VuePerfectScrollbar class="scroll-area--data-list-add-new pt-4 pb-6"> 
        ...
        <vs-input size="large" v-validate="'required'"   label="Cod Ubicacion" name="COD_UBICACION" v-model="COD_UBICACION" class="mt-5 w-full" />
         
        <span class="text-danger">{{errors.first("step-1.COD_UBICACION")}}</span>
      </div><div class="vx-col w-1/2">
        <vs-input size="large" v-validate="'required'"   label="Des Direccion" name="DES_DIRECCION" v-model="DES_DIRECCION" class="mt-5 w-full" />
         
        <span class="text-danger">{{errors.first("step-1.DES_DIRECCION")}}</span>
      </div>
                                                      <div class="vx-col md:w-1/2 w-full">
                                                <vs-input size="large" v-validate="{regex: '^([0-9]+)$' }" label="Must match the specified regular expression : ^[0-9]\d*(\.\d+)?$" name="LONGITUD" v-model="LONGITUD" class="mt-5 w-full" />
                                                
                                              </div>
                                                      <div class="vx-col md:w-1/2 w-full">
                                                <vs-input size="large" v-validate="{regex: '^([0-9]+)$' }" label="Must match the specified regular expression : ^[0-9]\d*(\.\d+)?$" name="LATITUD" v-model="LATITUD" class="mt-5 w-full" />
...
const dict = datos2509.mensajesValidacion;
...
export default {
  ...
  beforeMount() {
    ...
    this.defaultdatos = datos2509.datosDefault;
  ...
  watch: { 
    // SI HAY PROMPT
      ...
      this.COD_ZONA = this.rowselectionprompt.COD_ZONA;
      this.DI_ZONA = this.rowselectionprompt.DES_ZONA;
  }
  ...
  data() {
    return {
      ...
      // CMS CMS CMS CMS          DatOS cms Para manejo de EXPEDIENTE DIGITAL  MULTIMEDIA Debe venir en una funcion de Datos con la consulta
        // CMSDATA2509: datos2509.CMSDATA2509,  PROPSDataCMS: divilib.DataCMS,  tituloAvatar: "Para Cambiar Avatar",  verDocumento1: false,
  ...
  methods: {
    ...
    setActivaPromp2501() {  //GO_ZONAS
      var filtros2501 = [  
        { nombre: "nombre", valor:""},];

      this.$parent.cargaPrompt2501(filtros2501);
    }, 
 ...
 validateStep3() {
    ...
    // FIN DE VALIDA PASOS ESTO SI ES UN FORMULARIO DE PASOS 
     ...
     addFilas: function() {
      ...
      filas.push({
        ...
        VALOR1: this.COD_UBICACION,
        VALOR2: "",
        CDATA: "0"
      }); 
      filas.push({
        NOMBRE: "DES_DIRECCION",
        VALOR1: this.DES_DIRECCION,
        VALOR2: "",
        CDATA: "0"
      }); 
      filas.push({
    ...
    // CARGA DE una variable Bd a DOM
     cargaRegistroActual: function(datosRow) { 
          ...
          this.COD_UBICACION = datosRow.COD_UBICACION;
          this.DES_DIRECCION = datosRow.DES_DIRECCION;
   ...
   //cargaRegistroActual funcion completa   
    ...
    buscarRegistro: function(dinamicKey) {
      ...
      var origenUrl = "O";
       // agrego las filas
         var filas = datos2509.getMisFilas();
