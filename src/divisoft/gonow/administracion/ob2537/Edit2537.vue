    ...
    <VuePerfectScrollbar class="scroll-area--data-list-add-new pt-4 pb-6"> 
        <vx-input-group class="mb-base">
            <div class="vx-row"> 
                                ...
                                <vs-input size="large" v-validate="{regex: '^([0-9]+)$' }" label="Must match the specified regular expression : ^([0-9]+)$ - numbers only" name="COD_CATEGORIAMENSAJERO" v-model="COD_CATEGORIAMENSAJERO" class="mt-5 w-full" />
                                <span class="text-danger">{{errors.first("step-1.COD_CATEGORIAMENSAJERO")}}</span>
                              </div> <div class="vx-col md:w-1/2 w-full">  
                                       <ul class="centerx"  >
                                               <li  :key="index"  v-for="(item, index) in LI_COMISION" >
                                                      <vs-radio v-model="TIP_COMISIONMENSAJERO" :vs-value="item.value">{{item.text}}</vs-radio>
                                               </li>
                                          </ul> 
                                        
                                      </div>    
                                                      <div class="vx-col md:w-1/2 w-full">
                                                <vs-input size="large" v-validate="{regex: '^([0-9]+)$' }" label="Must match the specified regular expression : ^[0-9]\d*(\.\d+)?$" name="VALOR_COMISIONMENSAJERO" v-model="VALOR_COMISIONMENSAJERO" class="mt-5 w-full" />
                                                <span class="text-danger text-sm" v-show="errors.has('VALOR_COMISIONMENSAJERO')">{{ errors.first('VALOR_COMISIONMENSAJERO') }}</span>
                                              </div> <div class="vx-col md:w-1/2 w-full">  
                                       <ul class="centerx"  >
...
import * as datos2537 from "./js/divi2537.js";
...
const dict = datos2537.mensajesValidacion;
...
export default {
  ...
  beforeMount() {
    ...
    this.defaultdatos = datos2537.datosDefault;
  ...
  data() {
    return {
      ...
      // CMS CMS CMS CMS          DatOS cms Para manejo de EXPEDIENTE DIGITAL  MULTIMEDIA Debe venir en una funcion de Datos con la consulta
        // CMSDATA2537: datos2537.CMSDATA2537,  PROPSDataCMS: divilib.DataCMS,  tituloAvatar: "Para Cambiar Avatar",  verDocumento1: false,
      ...
      // CAMPOS DE LISTA  SI LOS HAHAY
       LI_COMISION :datos2537.LI_COMISION,LI_ESTADO :datos2537.LI_ESTADO,
 ...
 validateStep3() {
    ...
    // FIN DE VALIDA PASOS ESTO SI ES UN FORMULARIO DE PASOS 
     ...
     addFilas: function() {
      ...
      filas.push({
        ...
        VALOR1: this.COD_CATEGORIAMENSAJERO,
        VALOR2: "",
        CDATA: "0"
      }); 
      filas.push({
        NOMBRE: "TIP_COMISIONMENSAJERO",
        VALOR1: this.TIP_COMISIONMENSAJERO,
        VALOR2: "",
        CDATA: "0"
      }); 
      filas.push({
        NOMBRE: "VALOR_COMISIONMENSAJERO",
        VALOR1: this.VALOR_COMISIONMENSAJERO,
        VALOR2: "",
        CDATA: "0"
      }); 
      filas.push({
    ...
    // CARGA DE una variable Bd a DOM
     cargaRegistroActual: function(datosRow) { 
          this.COD_CATEGORIAMENSAJERO = datosRow.COD_CATEGORIAMENSAJERO;
          this.TIP_COMISIONMENSAJERO = datosRow.TIP_COMISIONMENSAJERO;
          this.VALOR_COMISIONMENSAJERO = datosRow.VALOR_COMISIONMENSAJERO;
   ...
   //cargaRegistroActual funcion completa   
    ...
    buscarRegistro: function(dinamicKey) {
      ...
      var origenUrl = "O";
       // agrego las filas
         var filas = datos2537.getMisFilas();
