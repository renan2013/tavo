    ...
    <VuePerfectScrollbar class="scroll-area--data-list-add-new pt-4 pb-6"> 
        <vx-input-group class="mb-base">
            <div class="vx-row"> 
                                ...
                                <vs-input size="large" v-validate="{regex: '^([0-9]+)$' }" label="Must match the specified regular expression : ^([0-9]+)$ - numbers only" name="COD_RANGO" v-model="COD_RANGO" class="mt-5 w-full" />
                                <span class="text-danger">{{errors.first("step-1.COD_RANGO")}}</span>
                              </div> 
                                 <div class="vx-col md:w-1/2 w-full">
                                <vs-input size="large" v-validate="{regex: '^([0-9]+)$' }" label="Must match the specified regular expression : ^([0-9]+)$ - numbers only" name="NUM_RANGOINICIO" v-model="NUM_RANGOINICIO" class="mt-5 w-full" />
                                <span class="text-danger">{{errors.first("step-1.NUM_RANGOINICIO")}}</span>
                              </div> 
                                 <div class="vx-col md:w-1/2 w-full">
                                <vs-input size="large" v-validate="{regex: '^([0-9]+)$' }" label="Must match the specified regular expression : ^([0-9]+)$ - numbers only" name="NUM_RANGOFINAL" v-model="NUM_RANGOFINAL" class="mt-5 w-full" />
                                <span class="text-danger">{{errors.first("step-1.NUM_RANGOFINAL")}}</span>
                              </div><div class="vx-col w-1/2">
        <vs-input size="large" v-validate="'required'"   label="Des Rango" name="DES_RANGO" v-model="DES_RANGO" class="mt-5 w-full" />
         
        <span class="text-danger">{{errors.first("step-1.DES_RANGO")}}</span>
...
import * as datos2523 from "./js/divi2523.js";
...
const dict = datos2523.mensajesValidacion;
...
export default {
  ...
  beforeMount() {
    ...
    this.defaultdatos = datos2523.datosDefault;
  ...
  data() {
    return {
      ...
      // CMS CMS CMS CMS          DatOS cms Para manejo de EXPEDIENTE DIGITAL  MULTIMEDIA Debe venir en una funcion de Datos con la consulta
        // CMSDATA2523: datos2523.CMSDATA2523,  PROPSDataCMS: divilib.DataCMS,  tituloAvatar: "Para Cambiar Avatar",  verDocumento1: false,
 ...
 validateStep3() {
    ...
    // FIN DE VALIDA PASOS ESTO SI ES UN FORMULARIO DE PASOS 
     ...
     addFilas: function() {
      ...
      filas.push({
        ...
        VALOR1: this.NUM_RANGOINICIO,
        VALOR2: "",
        CDATA: "0"
      }); 
      filas.push({
        NOMBRE: "NUM_RANGOFINAL",
        VALOR1: this.NUM_RANGOFINAL,
        VALOR2: "",
        CDATA: "0"
      }); 
      filas.push({
        NOMBRE: "DES_RANGO",
        VALOR1: this.DES_RANGO,
        VALOR2: "",
        CDATA: "0"
      }); 
      filas.push({
    ...
    // CARGA DE una variable Bd a DOM
     cargaRegistroActual: function(datosRow) { 
          ...
          this.NUM_RANGOINICIO = datosRow.NUM_RANGOINICIO;
          this.NUM_RANGOFINAL = datosRow.NUM_RANGOFINAL;
   ...
   //cargaRegistroActual funcion completa   
    ...
    buscarRegistro: function(dinamicKey) {
      ...
      var origenUrl = "O";
       // agrego las filas
         var filas = datos2523.getMisFilas();
