    ...
    <VuePerfectScrollbar class="scroll-area--data-list-add-new pt-4 pb-6"> 
        <vx-input-group class="mb-base">
            <div class="vx-row"> 
                                ...
                                <vs-input size="large" v-validate="{regex: '^([0-9]+)$' }" label="Must match the specified regular expression : ^([0-9]+)$ - numbers only" name="COD_MARCA" v-model="COD_MARCA" class="mt-5 w-full" />
                                <span class="text-danger">{{errors.first("step-1.COD_MARCA")}}</span>
                              </div><div class="vx-col w-1/2">
        <vs-input size="large" v-validate="'required'"   label="Des Marca" name="DES_MARCA" v-model="DES_MARCA" class="mt-5 w-full" />
         
        <span class="text-danger">{{errors.first("step-1.DES_MARCA")}}</span>
      </div> 
                                 <div class="vx-col md:w-1/2 w-full">
...
import * as datos2524 from "./js/divi2524.js";
...
const dict = datos2524.mensajesValidacion;
...
export default {
  ...
  beforeMount() {
    ...
    this.defaultdatos = datos2524.datosDefault;
  ...
  data() {
    return {
      ...
      // CMS CMS CMS CMS          DatOS cms Para manejo de EXPEDIENTE DIGITAL  MULTIMEDIA Debe venir en una funcion de Datos con la consulta
        // CMSDATA2524: datos2524.CMSDATA2524,  PROPSDataCMS: divilib.DataCMS,  tituloAvatar: "Para Cambiar Avatar",  verDocumento1: false,
 ...
 validateStep3() {
   ...
   //cargaRegistroActual funcion completa   
    ...
    buscarRegistro: function(dinamicKey) {
      ...
      var origenUrl = "O";
       // agrego las filas
         var filas = datos2524.getMisFilas();
