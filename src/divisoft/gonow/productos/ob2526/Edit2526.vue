    ...
    <VuePerfectScrollbar class="scroll-area--data-list-add-new pt-4 pb-6"> 
        <vx-input-group class="mb-base">
            <div class="vx-row"> 
                                ...
                                <vs-input size="large" v-validate="{regex: '^([0-9]+)$' }" label="Must match the specified regular expression : ^([0-9]+)$ - numbers only" name="COD_CATEGORIA" v-model="COD_CATEGORIA" class="mt-5 w-full" />
                                <span class="text-danger">{{errors.first("step-1.COD_CATEGORIA")}}</span>
                              </div><div class="vx-col w-1/2">
        <vs-input size="large" v-validate="'required'"   label="Des Categoria" name="DES_CATEGORIA" v-model="DES_CATEGORIA" class="mt-5 w-full" />
         
        <span class="text-danger">{{errors.first("step-1.DES_CATEGORIA")}}</span>
      </div> 
                                 <div class="vx-col md:w-1/2 w-full">
                                <vs-input size="large" v-validate="{regex: '^([0-9]+)$' }" label="Must match the specified regular expression : ^([0-9]+)$ - numbers only" name="IMG_AVATAR" v-model="IMG_AVATAR" class="mt-5 w-full" />
                
                              </div><div class="vx-col w-1/2">
        <vs-input size="large"    label="Hash Avatar" name="HASH_AVATAR" v-model="HASH_AVATAR" class="mt-5 w-full" /> 
...
import * as datos2526 from "./js/divi2526.js";
...
const dict = datos2526.mensajesValidacion;
...
export default {
  ...
  beforeMount() {
    ...
    this.defaultdatos = datos2526.datosDefault;
  ...
  data() {
    return {
      ...
      // CMS CMS CMS CMS          DatOS cms Para manejo de EXPEDIENTE DIGITAL  MULTIMEDIA Debe venir en una funcion de Datos con la consulta
        // CMSDATA2526: datos2526.CMSDATA2526,  PROPSDataCMS: divilib.DataCMS,  tituloAvatar: "Para Cambiar Avatar",  verDocumento1: false,
 ...
 validateStep3() {
    ...
    // CARGA DE una variable Bd a DOM
     cargaRegistroActual: function(datosRow) { 
          ...
          this.DES_CATEGORIA = datosRow.DES_CATEGORIA;
   ...
   //cargaRegistroActual funcion completa   
    ...
    buscarRegistro: function(dinamicKey) {
      ...
      var origenUrl = "O";
       // agrego las filas
         var filas = datos2526.getMisFilas();
