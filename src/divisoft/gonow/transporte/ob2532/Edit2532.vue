    ...
    <VuePerfectScrollbar class="scroll-area--data-list-add-new pt-4 pb-6"> 
        <vx-input-group class="mb-base">
            <div class="vx-row"> 
                                ...
                                <vs-input size="large" v-validate="{regex: '^([0-9]+)$' }" label="Must match the specified regular expression : ^([0-9]+)$ - numbers only" name="NUM_TRANSPORTE" v-model="NUM_TRANSPORTE" class="mt-5 w-full" />
                                <span class="text-danger">{{errors.first("step-1.NUM_TRANSPORTE")}}</span>
                              </div><div class="vx-col w-1/2">
        <vs-input size="large" v-validate="'required'"   label="Des Transporte" name="DES_TRANSPORTE" v-model="DES_TRANSPORTE" class="mt-5 w-full" />
         
        <span class="text-danger">{{errors.first("step-1.DES_TRANSPORTE")}}</span>
...
import * as datos2532 from "./js/divi2532.js";
...
const dict = datos2532.mensajesValidacion;
...
export default {
  ...
  beforeMount() {
    ...
    this.defaultdatos = datos2532.datosDefault;
  ...
  data() {
    return {
      ...
      // CMS CMS CMS CMS          DatOS cms Para manejo de EXPEDIENTE DIGITAL  MULTIMEDIA Debe venir en una funcion de Datos con la consulta
        // CMSDATA2532: datos2532.CMSDATA2532,  PROPSDataCMS: divilib.DataCMS,  tituloAvatar: "Para Cambiar Avatar",  verDocumento1: false,
 ...
 validateStep3() {
    ...
    // CARGA DE una variable Bd a DOM
     cargaRegistroActual: function(datosRow) { 
          ...
          this.DES_TRANSPORTE = datosRow.DES_TRANSPORTE;
   ...
   //cargaRegistroActual funcion completa   
    ...
    buscarRegistro: function(dinamicKey) {
      ...
      var origenUrl = "O";
       // agrego las filas
         var filas = datos2532.getMisFilas();
