            ...
            <div class="vx-row mt-6">
              <div class="vx-col md:w-2/5 w-full flex items-center justify-center">
                <div class="product-img-container w-3/5 mx-auto mb-10 md:mb-0">
                  <img :src="item_data.image2" :alt="item_data.name" class="responsive">
          ...
          <swiper :options="swiperOption" :dir="$vs.rtl ? 'rtl' : 'ltr'" :key="$vs.rtl" class="related-product-swiper px-12 py-6">
            <swiper-slide v-for="item in related_items" :key="item.objectId" class="p-6 rounded cursor-pointer">
              ...
              <div class="img-container w-32 mx-auto my-base">
                <img class="responsive" :src="item.image2" :alt="item.name">
              </div>
...
export default{
  ...
  created () {
     ...
     let codigo=this.$route.params.item_id;
  
    if (codigo<0)
      {  
       codigo=this.getUltimoCodigo 
       this.fetch_item_details(codigo)
      }
    else
    {
      
      //guarda el ultimo
      this.$store.commit("eCommerce/MUTSETULTIMOPRODUCTO",codigo);
      this.fetch_item_details(codigo)
  ...
  computed: {
    ...
    related_items() {
      return this.$store.state.eCommerce.related_items
    },       
    item_data () {
      return this.$store.state.eCommerce.ProductsRow
    },   
    getUltimoCodigo () {
      return this.$store.state.eCommerce.ulimoProducto
    },   
    item_qty () {
  ...
  methods: {
    ...
    fetch_item_details (id) {
      this.buscarDatos(9,id);  // busca uno indivisual
      this.buscarDatos(10,id);
    },

    buscarDatos: function (indicador,codigo) { 
            var origenUrl = "O";
            ...
            parametro1 = { NOMBRE: "PN_PARTNER",OPERADOR: "=",VALOR1: '5069',VALOR2: '',CDATA: '0'};
            parametros.push(parametro1); 


            parametro1 = { NOMBRE: "PN_CODPRODUCTO",OPERADOR: "=",VALOR1: codigo,VALOR2: '',CDATA: '0'};
            parametros.push(parametro1);
